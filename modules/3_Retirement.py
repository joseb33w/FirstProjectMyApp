...
def main():
    ...
    if st.button("Calculate Retirement Savings"):
        ...
        if retirement_age <= current_age:
            ...
            # Create Altair line chart with zoom and scroll enabled
            line_chart = alt.Chart(savings_df).mark_line(point=True).encode(
                x=alt.X(
                    "Age:Q", 
                    title="Age", 
            ...
            ).interactive(bind_y=False)  # Enable zoom and scroll interactively, fixed y-axis range

            # Display the line chart below the calculation result
