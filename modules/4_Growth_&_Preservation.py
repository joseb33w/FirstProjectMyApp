...
if "annual_contribution" not in st.session_state:
    st.session_state.annual_contribution = 5000
...
# Calculate projected retirement income and savings longevity with contributions
def calculate_retirement_insights():
    ...
    # Including annual contributions in the future value calculation
    for year in range(years_until_retirement):
        future_value *= (1 + st.session_state.annual_return / 100)
        future_value += st.session_state.annual_contribution
...
def retirement_tracker():
    ...
    st.session_state.annual_contribution = st.number_input("Annual Contribution ($):", min_value=0, value=5000)
