...
def main():
    st.text_input("Search Topics", key="search_bar")
    
    # Single-row layout for buttons with reduced space
    col_a, col_b, col_c, col_d, col_e = st.columns([1, 1, 1, 1, 1])

    # Button selection with session state management in a single row
    with col_a:
        if st.button("Topics"):
            st.session_state.selected_topic = "All"
    with col_b:
