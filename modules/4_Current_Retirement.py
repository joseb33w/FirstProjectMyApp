    ...
    if current_savings is not None and dates:
        ...
        for date in dates:
            try:
                ...
                years.append(parsed_date.strftime('%m/%d'))
            except ValueError:
                st.error(f"Invalid date format: {date}")

        selected_year = st.selectbox("Select Year", sorted(set(years)))

        # Filter data for the selected year
        filtered_dates = [date for date in dates if parser.parse(date).strftime('%m/%d') == selected_year]
        filtered_savings = [savings[i] for i, date in enumerate(dates) if parser.parse(date).strftime('%m/%d') == selected_year]
