    ...
    if st.button("Calculate Retirement Savings"):
        ...
        if retirement_age <= current_age:
            ...
            bar_chart = alt.Chart(savings_df).mark_bar().encode(
                ...
                y=alt.Y("Savings:Q", scale=alt.Scale(domain=[0, max_savings]), title="Savings ($)", axis=alt.Axis(labelExpr="datum.value >= 1000000 ? datum.value / 1000000 + 'M' : datum.value >= 1000 ? datum.value / 1000 + 'k' : datum.value")),
                tooltip=[alt.Tooltip("Age:Q", title="Age"), alt.Tooltip("Savings:Q", title="Savings", format="$.2f")]
            ).properties(
                width=800,
                height=400,
