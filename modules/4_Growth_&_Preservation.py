...
# Calculate projected retirement income and savings longevity with annual compounding
def calculate_retirement_insights():
    years_until_retirement = st.session_state.retirement_age - st.session_state.user_age
    future_value = st.session_state.savings_amount

    # Annual compounding with monthly contributions
    for year in range(years_until_retirement):
        future_value *= (1 + st.session_state.annual_return / 100)
        future_value += st.session_state.monthly_contribution * 12

    monthly_income = future_value * (st.session_state.annual_return / 100) / 12

    # Adjusting expenses for inflation annually
    adjusted_monthly_expenses = st.session_state.monthly_expenses
    for year in range(years_until_retirement):
        adjusted_monthly_expenses *= (1 + st.session_state.inflation_rate / 100)
...
def retirement_tracker():
    ...
    st.session_state.monthly_contribution = st.number_input("Monthly Contribution ($):", min_value=0, value=500)
    ...
    # Updated Retirement Readiness Indicator
    if years_covered >= 35:
    ...
    elif years_covered >= 10:
        st.error("❌ Your current savings may not be sufficient for a comfortable retirement.")
    else:
        st.error("❌❌ Critical alert! Your savings are severely insufficient for retirement.")
