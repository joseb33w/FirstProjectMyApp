...
def main():
    ...
    if current_savings is not None and dates:
        ...
        filtered_dates = [date for date in dates if datetime.strptime(date, "%Y-%m-%d").year == selected_year]
        filtered_savings = [savings[i] for i, date in enumerate(dates) if datetime.strptime(date, "%Y-%m-%d").year == selected_year]
