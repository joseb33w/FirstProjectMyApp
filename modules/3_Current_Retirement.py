...
def display_savings_chart(data):
    ...
    total_return = sum(investment['return'] for investment in investments)
    total_percentage = sum(investment['percentage'] for investment in investments) / len(investments)

    for investment in investments:
        st.markdown(f"**{investment['name']}:**\n${investment['return']:,.2f}\n{investment['percentage']:.2f}%")

    # Display investment returns summary as the average of individual investments
    st.markdown("<h3 style='font-size:28px;'>Investment's Return</h3>", unsafe_allow_html=True)
    st.markdown(f"**Average Return on Investment:**\n${total_return:,.2f}\n{total_percentage:.2f}%")

    # Display current expenses adjusted for inflation
    st.markdown("<h3 style='font-size:28px;'>Current Expenses (Inflation Adjusted)</h3>", unsafe_allow_html=True)
    current_expenses = 3000  # Example current monthly expense
    inflation_rate = 3.0  # Example inflation rate as a percentage
    inflation_adjusted_expenses = current_expenses * 12 * (1 + inflation_rate / 100)
    st.markdown(f"**Monthly Expenses:** ${current_expenses:,.2f}")
    st.markdown(f"**Inflation Rate:** {inflation_rate:.2f}%")
    st.markdown(f"**Inflation-Adjusted Annual Expenses:** ${inflation_adjusted_expenses:,.2f}")
